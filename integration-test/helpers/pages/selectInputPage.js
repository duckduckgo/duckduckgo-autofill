...
export function selectInputPage (page) {
    class SelectInputPage {
        ...
        async selectOption (option, withLabel) {
            await page.selectOption(withLabel ? `form#with-label select#address-city` : `form#no-label select#address-city`, {value: option})
        }
        ...
        async assertSelectedValue (option, withLabel = false) {
            const selectValue = await page.locator(withLabel ? `form#with-label select#address-city` : 'form#no-label select#address-city').inputValue()
