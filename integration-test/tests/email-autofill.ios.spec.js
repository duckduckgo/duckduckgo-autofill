...
test.describe('ios', () => {
    test('should autofill the selected email when email protection is enabled', async ({ page }) => {
        ...
        await createWebkitMocks('ios').withAvailableInputTypes({ email: true }).withPersonalEmail('0').withPrivateEmail('0').applyTo(page)

        // Load the autofill.js script with replacements
        await createAutofillScript()
            .replaceAll(
                iosContentScopeReplacements({
    ...
    test('should not autofill email when email protection is disabled', async ({ page }) => {
        ...
        await createWebkitMocks('ios').applyTo(page)
