...
class Matching {
  ...
  inferPasswordVariant(input, opts) {
    ...
    if (autocompleteAttr) {
      if (((0, _autofillUtils.safeRegexTest)(/new.?password|password.?new/i, autocompleteAttr) || (0, _autofillUtils.safeRegexTest)(/new.?password|password.?new/i, nameAttr) || (0, _autofillUtils.safeRegexTest)(/new.?password|password.?new/i, idAttr)) && opts.isSignup) {
        return 'credentials.password.new';
      }
      if (((0, _autofillUtils.safeRegexTest)(/current.?password|password.?current/i, autocompleteAttr) || (0, _autofillUtils.safeRegexTest)(/current.?password|password.?current/i, nameAttr) || (0, _autofillUtils.safeRegexTest)(/current.?password|password.?current/i, idAttr)) && (opts.isLogin || opts.isHybrid)) {
