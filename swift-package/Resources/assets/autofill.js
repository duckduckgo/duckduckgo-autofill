...
function createCredentialsTooltipItem(data) {
  // TODO: if (PROVIDER_LOCKED in data) create the vendor locked item
...
class DataHTMLTooltip extends _HTMLTooltip.default {
  ...
  render(config, items, callbacks) {
    ...
    const dataTypeClass = "tooltip__button--data--".concat(config.type); // TODO: instead of reading from the settings here this piece of data should come from the items

    const providerIconClass = credentialsProvider ? "tooltip__button--data--".concat(credentialsProvider) : '';
...
class HTMLTooltipUIController extends _UIController.UIController {
  ...
  createTooltip(getPosition, topContextData) {
    ...
    const tooltipOptions = { ...this._htmlTooltipOptions,
      ...
      credentialsProvider: topContextData.credentialsProvider,
