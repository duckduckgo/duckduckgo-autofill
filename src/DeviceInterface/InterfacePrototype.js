...
class InterfacePrototype {
    ...
    /** @type {string | null} */
    lastEmailProtectionAddressUsed = null;
    ...
    postAutofill (data, dataType, formObj) {
        ...
        if (
            ...
            if (formValues.credentials?.password === data.password) {
                const formData = prepareForFormStorage(formValues, 'passwordGeneration', data.password, undefined)
        ...
        if ((data?.id === 'privateAddress' || data?.id === 'personalAddress' || data?.id === 'emailProtectionAddress')
            ...
            if (!this.globalConfig.isMobileApp && data.id === 'privateAddress') {
                ...
                const formData = prepareForFormStorage(formValues, 'emailProtection', undefined, undefined)
                this.storeFormData(formData)
            }
    ...
    postSubmit (values, form) {
        ...
        if (checks.some(Boolean)) {
            const formData = prepareForFormStorage(values, 'formSubmission', this.passwordGenerator.password, this.lastEmailProtectionAddressUsed)
            this.storeFormData(formData)
        }
    }
