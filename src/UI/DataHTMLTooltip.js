...
class DataHTMLTooltip extends HTMLTooltip {
    ...
    render(device, config, items, callbacks) {
        ...
        const shouldShowManageButton =
            isTopAutofill && items.some((item) => !['personalAddress', 'privateAddress', PROVIDER_LOCKED].includes(item.id()));
    ...
    <div class="tooltip tooltip--data${this.options.isIncontextSignupAvailable() ? ' tooltip--incontext-signup' : ''}">
        ${items
            ...
            <button id="${item.id()}" class="tooltip__button tooltip__button--data ${dataTypeClass} ${paymentProviderIconClass} ${providerIconClass} js-autofill-button ${disableHoverEffectClass}">
                <span class="tooltip__button__text-container">
                    <span class="label label--medium truncate" title="${escapeXML(item.labelMedium(t, this.subtype))}">${escapeXML(item.labelMedium(t, this.subtype))}</span>
